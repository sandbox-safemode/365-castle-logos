...
// Generates a brand-new castle logo from a text prompt via Gemini (no source image).
// Uploads result to Supabase Storage, stores row in castle_creations table.
...
export default async function handler(req, res) {
  ...
  // 2. Call Gemini for text-to-image generation (no source image)
  let resultMime;
  let resultBuffer;
  try {
    const geminiPayload = {
  ...
  try {
    ...
    const { error: uploadErr } = await sb.storage
      .from(STORAGE_BUCKET)
      .upload(filename, resultBuffer, { contentType: resultMime, upsert: true });
  ...
  // 4. Save public URL to castle_edits
  ...
  return res.status(200).json({ jobId, status: 'done', imageUrl: publicUrl });
}
